...
NodeCG.waitForReplicants(tweetData, mediaData, selectedRunId).then(() => {
	...
	mediaData.on('change', (newVal) => {
		...
		newVal.forEach((media) => {
			...
			paperItem.dataset.mediaName = media.base; // Makes sure to prevent url encoding :)
...
document.addEventListener('dialog-confirmed', () => {
	...
	tweetData.value[selectedRunId.value].media = document.getElementById('mediaList').selectedItem.dataset.mediaName;
});
